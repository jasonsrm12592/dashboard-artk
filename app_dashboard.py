...
def cargar_inventario_baja_rotacion():
    """
    V8.2: Rotación Real (Huesos por Falta de Salidas).
...
with tab_inv:
    ...
    if not df_huesos.empty:
        ...
        items_totales = len(df_huesos) # Universo Total
        items_hueso = len(df_show)     # Huesos Filtrados
        
        m1, m2, m3 = st.columns(3)
        m1.metric(f"Capital Estancado (>{dias_min} días)", f"₡ {total_atrapado/1e6:,.1f} M")
        m2.metric("Total Items en Bodega", items_totales)
        m3.metric("Items Hueso (Filtrados)", items_hueso, delta_color="inverse")
